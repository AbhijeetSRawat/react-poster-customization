...
const CoustomDesign = ({ mode, setMode }) => {
  ...
  const reqData = allData.filter(pic => pic.category === 'Custom Design');
  ...
  return (
    <div className={`min-h-screen pt-16 ${mode ? "bg-blue-300 text-gray-700" : "bg-slate-900 text-gray-400"}`}>
      ...
      <div className="mt-5 px-4">
        <h2 className="text-xl font-semibold mb-3">Custom Design</h2>
        {allLoading ? (
          ...
          <div className="flex flex-wrap gap-4 pb-4">
            {reqData.map((pic, index) => (
              ...
              >
                <img
                  src={pic.path}
                  alt={pic.productImage || "Custom Design"}
