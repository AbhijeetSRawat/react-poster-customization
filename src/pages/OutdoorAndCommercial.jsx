...
const OutdoorAndCommercial = ({ mode, setMode }) => {
  ...
  const reqData = allData.filter((pic) => pic.category === 'Outdoor & Commercial');
  ...
  return (
    <div className={`min-h-screen pt-16 ${mode ? "bg-blue-300 text-gray-700" : "bg-slate-900 text-gray-400"}`}>
      ...
      {selectedImage && (
        <div className="flex justify-center mb-4">
          <button
            ...
            className="bg-blue-600 mt-8 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded shadow transition"
          >
            Download
          </button>
        </div>
      ...
      <div className="mt-5 px-4">
        <h2 className="text-xl font-semibold mb-3">Outdoor And Commercial</h2>
        {allLoading ? (
          ...
          <div className="flex flex-wrap gap-4 pb-4">
            {reqData.map((pic, index) => (
              ...
              >
                <img
                  src={pic.path}
                  alt={pic.productImage || "Outdoor & Commercial"}
...
export default OutdoorAndCommercial;
